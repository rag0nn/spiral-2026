...
class DetCriterion(nn.Module):
    """
    Object detection basligi icin kayip fonksiyonu
    """
...
class PosCriterion(nn.Module):
    """
    Translation / Konum basligi icin kayip fonksiyonu
    """
...
class SpiLoss(nn.Module):
    """
    Modelin her iki basligi icin ortak kayip yoneticisi
    """
    def __init__(self, num_classes=4, mode="multi_task", lambda_pos=1.0, alpha=0.25, gamma=2.0):
        super().__init__()
        self.mode = mode
        self.lambda_pos = lambda_pos
        self.det_criterion = DetCriterion(num_classes=num_classes, alpha=alpha, gamma=gamma)
